var slideshow = (function(){
    ...
    var galleryMaxHeight = Infinity; // height of entire gallery
    var calcGalleryMaxHeight;
    ...
    function setupGallery(options){
        ...
        maxThumbnailHeight = options.maxThumbnailHeight || maxThumbnailHeight;
        calcGalleryMaxHeight = options.calcGalleryMaxHeight || calcGalleryMaxHeight;
        onComplete = options.onComplete || onComplete;
        slideshowIndex = -1;    // gallery mode
        $("#JustifiedGallery").addClass("galleryMode");
        init();
    }
    function setupSlideshow(URL, Animations, usingAjax){
        loadURL = URL;
        ...
        $("#slideshowBox").mouseout(function(e){
            next.css({opacity:0});
            prev.css({opacity:0});
        });
        initSwipe();
    ...
    function nextOrPrev(e){
        if(jQuery(e.target).attr("id")=="next" || jQuery(e.target).parents(".arrow").attr("id")=="next")    return "next"; //Next arrow
        ...
        else if (e.originalEvent.changedTouches[0].pageX - ($(window).width()-$("#slideshowBox").width())/2 < $("#slideshowBox").width()/4) return "prev"; // tapped left 1/4 of image
        else return "none";
    }
    ...
    function init(){
        ...
        $(window).resize(function() {
            ...
            if (nowWidth < lastWidth || nowWidth > lastWidth || nowHeight > lastHeight+30 || nowHeight < lastHeight-30){
                ...
                if (calcGalleryMaxHeight) galleryMaxHeight = calcGalleryMaxHeight();
                justifiedGallery($("#Gallery"),galleryMaxHeight, 0,Infinity, maxThumbnailHeight);
        ...
        if (desiredSlideIndex>slideshowIndex){  // next slide transition
            ...
            $("#slideshowBox").animate({
                ...
                function(){
                    ...
                    diff--;
                    slideshow();
                    if (diff>0) slideTransition(diff);
                });
    ...
    function initSwipe(){
        $(".slideshowBox").on('touchstart mousedown',function(e) {
            ...
            if (e.type=="touchstart" || e.which == 1){  // touch or left mouse button
                ...
                $(".slideshowBox").live("touchmove mousemove", function (e){
            ...
            if (startTime!=-1){ // currently swiping
                ...
                if(Math.abs(distanceMoved) > $(window).width()/4    //swiped far enough
                    ...
                    if(direction=="next"){  //Click Next
                        nextSlide();
                    }else if(direction=="prev"){    //Click Previous
                        previousSlide();
    ...
    function showNextImages() {
        justifiedGallery($("#Gallery"),galleryMaxHeight, lastRowIndex, Infinity, maxThumbnailHeight);
    }
    ...
    function loadMoreImages(){
        if (useAjax && !reachedBottom && !loading){
            loading = 1;
            $.ajax({
                url: loadURL,
                data:{ 
